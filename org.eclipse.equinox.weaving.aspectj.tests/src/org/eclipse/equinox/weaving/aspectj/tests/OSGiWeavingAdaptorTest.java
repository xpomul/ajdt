...
public class OSGiWeavingAdaptorTest extends TestCase {
    ...
    public void testGeneratedClasses() throws Exception {
        ...
        final OSGiWeavingAdaptor adaptor = new OSGiWeavingAdaptor(null, null,
                "dummy");
        ...
        Field generatedClassHandlerField = WeavingAdaptor.class
                .getDeclaredField("generatedClassHandler");
        ...
        getWovenBytes.invoke(adaptor, "my.foo.MyClass", classBytes);

        // - and then the generated classes are read
        Map<String, byte[]> actualResult = adaptor.getGeneratedClassesFor("my.foo.MyClass");

        // THEN:
        // - the result should not include the non-generated class, only the three generated ones
        ...
        assertEquals("Expected size 3 of result " + actualContents.toString(),
                3, actualResult.size());
    }
}
